...
const handleBasicAuth = async (req) => {
    ...
    if (process.env.SYNGRISI_AUTH !== '1') {
        ...
        await req.logIn(guest, (err) => {
            if (err) {
                ...
                result.status = 301;
                result.value = `/login?=Error: cannot find quest user: ${err}`;
            } else {
                result.type = 'success';
                result.status = 200;
                result.user = guest;
    ...
    if (process.env.SYNGRISI_AUTH === '1'
        ...
        result.type = 'redirect';
        result.status = 301;
        result.value = '/first_run_password?admin=true';
        return result;
    }
    ...
    if (process.env.SYNGRISI_AUTH === '1') {
        ...
        if (req?.originalUrl !== '/') {
            result.value = `/login?origin=${encodeURIComponent(req.originalUrl)}`;
            return result;
        }
...
exports.ensureLoggedInOrApiKey = () => {
    return async (req, res, next) => {
