    ...
    return function (req, res, next) {
        if (process.env.SYNGRISI_AUTH !== '1') {
            User.findOne({ username: 'Guest' }).then((guest) => {
                if(!guest){
                    return res.status(401).json({error: 'cannot find user Guest'})
                }
                req.logIn(guest, (err) => {
                    if (err) {
                        console.log({ err });
...
exports.ensureApiKey = function ensureApiKey() {
    return function (req, res, next) {
        if (!req.headers.apikey) {
            return res.status(401).json({ error: 'there is no API key' });
        }
        ...
        if (process.env.SYNGRISI_AUTH !== '1') {
            ...
            User.findOne({ apiKey: req.headers.apikey }).then((usr) => {
