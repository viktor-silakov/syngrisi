...
async function updateItem(itemType, filter, params) {
    ...
    await item.update(params)
        ...
        .catch(function (err) {
            console.log(`Cannot update item type: '${itemType}', filter: '${JSON.stringify(filter)}', params: '${JSON.stringify(params)}'`);
        });
}
...
async function updateItemDate(mdClass, id) {
    return new Promise(async function (resolve, reject) {
        ...
        const item = await mongoose.model(mdClass)
            .findById(id);
        resolve( await item.update({Updated_date: Date.now()}).exec() )
            .catch(function (err) {
                console.log(`Cannot update date for '${mdClass}': id: '${id}' error: '${err}'`);
                reject(err);
            });
        console.log(`'${mdClass}' date updated: '${JSON.stringify(updatedItem)}'`);
    })
...
async function createItem(modelName, params) {
    const Item = mongoose.model(modelName);
    const test = await Item.findOne({name: params.name});
    console.log(`create new '${modelName}', params: '${JSON.stringify(params)}'`);
    return Item.create(params)
        .then(async function (tst) {
            return tst.save()
                .then(async function (savedTest) {
...
// exports.createTestIfNotExist = async function createTestIfNotExist(params) {
//     return createItemIfNotExist('VRSTest', params);
// };
