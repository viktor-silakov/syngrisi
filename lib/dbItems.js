...
const createItemIfNotExistAsync = async function (modelName, params, logsMeta = {}) {
    ...
    try {
        ...
        if (item) {
            log.debug(`ORM item '${modelName}' already exists, params: '${JSON.stringify(params)}'`, $this, logsMeta);
            return item;
        }
        log.debug(`ORM item '${modelName}' does not exists, create new one, params: '${JSON.stringify(params)}'`, $this, logsMeta);
        const newItem = await itemModel.create(params);
        // await newItem.save();
        log.info(`ORM item '${modelName}' was created: '${JSON.stringify(newItem)}'`, $this, { ...{ ref: newItem._id }, ...logsMeta });
        return newItem;
    } catch (e) {
        log.debug(`cannot create '${modelName}' ORM item, error: '${e}'`, $this, logsMeta);
    }
    return null;
};
