...
async function createItemIfNotExist(modelName, params) {
    ...
    if (test) {
        log.debug(`'${modelName}' already exists, params: '${JSON.stringify(params)}'`, $this);
        return test;
    }
    log.debug(`'${modelName}' does not exists, create new one, params: '${JSON.stringify(params)}'`, $this);
    return Item.create(params)
        .then(async function (tst) {
            return tst.save()
                .then(async function (savedTest) {
                    log.debug(`'${modelName}' was created: '${JSON.stringify(savedTest)}'`, $this);
                    return savedTest;
                });
        })
        .catch(function (err) {
...
const createItemIfNotExistSync = function createItemIfNotExistSync(modelName, params) {
    const Item = mongoose.model(modelName);
    return Item.findOne(params)
        .then((item) => {
            ...
            return Item.create(params)
                .then((it) => it.save())
                .then((savedIt) => {
                    log.debug(`'${modelName}' was created: '${JSON.stringify(savedIt)}'`, $this);
                    return Promise.resolve(savedIt);
                })
                .catch((err) => {
                    log.debug(`cannot create '${modelName}', error: '${err}'`, $this);
                    return Promise.reject(new Error(`Cannot create '${modelName}', error: '${err}'`));
                });
        });
};

exports.createItemIfNotExistSync = createItemIfNotExistSync;

async function createItem(modelName, params) {
    const Item = mongoose.model(modelName);
    // const test = await Item.findOne({name: params.name});
    log.debug(`START CREATE ORM ITEM '${modelName}', params: '${JSON.stringify(params)}'`, $this);
    return Item.create(params)
        .then(async function (tst) {
            return tst.save()
                .then(async function (savedTest) {
                    log.debug(`CREATED ORM ITEM: '${modelName}' was created: '${JSON.stringify(savedTest)}'`, $this);
                    return savedTest;
                });
        })
        .catch(function (err) {
...
function createItemProm(modelName, params) {
    return new Promise(async (resolve, reject) => {
        try {
            const Item = mongoose.model(modelName);
            log.debug(`START CREATE ORM ITEM PROMISE'${modelName}', params: '${JSON.stringify(params)}'`, $this);
...
exports.createRunIfNotExist = async function createRunIfNotExist(params) {
    ...
    return createItemIfNotExistSync('VRSRun', params);
};
