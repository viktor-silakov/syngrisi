...
const getAllElementsByPositionFromDump = function getAllElementsByPositionFromDump(dump, positions) {
    return new Promise(function (resolve, reject) {
        try {
            ...
            console.log(`Promises formed, time: ${process.hrtime(executionTimer)
                .toString()}`);

            let dumpResults = {};
            Promise.all(promResults)
                .then((values) => {
                    console.log(`Promises resolved, time: ${process.hrtime(executionTimer)
                        .toString()}`);
                    for (el of values) {
                        // to make dump unique
                        if (el !== null) {
                    ...
                    console.log(`End parse elements from dump based on diff, time: '${process.hrtime(executionTimer)
                        .toString()}'`);
                    return resolve(Object.values(dumpResults)
                        .map(x => x));
                });
        } catch (e) {
            reject(e);
