...
function getDiff(baselineBufferPromise, actualBufferPromise, opts = {}) {
    return new Promise(async function (resolve, reject) {
        try {
            ...
            // direct comparison
            console.log(`SAMPLE #1: ${process.hrtime(executionTimer)
                .toString()}`);

            const directDiff = await makeDiff(baselineOrigin, actualOrigin, opts);
            console.log(`SAMPLE #2: ${process.hrtime(executionTimer)
                .toString()}`);
            ...
            console.log(`SAMPLE #3: ${process.hrtime(executionTimer)
                .toString()}`);
