...
async function compareWithStabilization(baselineOrigin, actualOrigin, method, opts) {
    return new Promise(async function (resolve, reject) {
        try {
            ...
            log.debug(`Error in compareWithTopShifting: '${e}'`, $this);
            return reject(e);
        }
    });
}
            ...
            Promise.all(diffs)
                .then((values) => {
                    ...
                    moreFittingResult.executionTotalTime = process.hrtime(executionTimer)
                        .toString();
                    log.debug(moreFittingResult, $this);
                    return resolve(moreFittingResult);
                });
        } catch (e) {
            log.error(`error in getDiff: '${e}'`, $this);
            log.debug(e.stack ? e.stack.split('\n') : e, $this);
            return reject(e);
        }
    });
}
