...
Feature: VRS Few checks with same identification
    ...
    Scenario: VRS two passed checks with same ident - passed ident group, passed test
        ...
        When I click on the element "//a[@name='group-count-link' and text()='(2)']"
        When I wait for "3" seconds
        Then I expect that element "img[title='baseline snapshoot']" does appear exactly "2" times
        Then I expect that element "img[title='actual snapshoot']" does appear exactly "2" times
        Then I expect that element "img[title='diff snapshoot']" does appear exactly "0" times
    ...
    Scenario: VRS two failed checks with same ident - failed ident group, failed test
        ...
        Then I expect that element "img[title='baseline snapshoot']" does appear exactly "2" times
        Then I expect that element "img[title='actual snapshoot']" does appear exactly "2" times
        Then I expect that element "img[title='diff snapshoot']" does appear exactly "2" times
    ...
    Scenario: VRS first failed check and second passed with same ident - blinking ident group, passed test
        ...
        Then I expect that element "img[title='baseline snapshoot']" does appear exactly "2" times
        Then I expect that element "img[title='actual snapshoot']" does appear exactly "2" times
        Then I expect that element "img[title='diff snapshoot']" does appear exactly "1" times
