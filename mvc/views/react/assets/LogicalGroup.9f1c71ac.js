...
function groupOptions({ data: data2 }) {
  const sortedData = [];
  const unGroupedData = [];
  const groupedData = data2.reduce((acc, item, index) => {
    if (item.group) {
      if (acc[item.group])
  ...
  Object.keys(groupedData).forEach((groupName) => {
    sortedData.push(...groupedData[groupName].map((index) => data2[index]));
  });
  sortedData.push(...unGroupedData.map((itemIndex) => data2[itemIndex]));
  return sortedData;
}
function getGroupedOptions(data2) {
  const sorted = groupOptions({ data: data2 });
  const unGrouped = [];
  const grouped = [];
  let groupName = null;
...
function SelectItems({
  ...
  if (creatable) {
    const creatableDataItem = data2[creatableDataIndex];
    unGroupedItems.push(
      /* @__PURE__ */ jsx("div", {
        className: classes.item,
...
const SegmentedControl = react.exports.forwardRef((props, ref) => {
  ...
  const shouldReduceMotion = useReducedMotion();
  const reduceMotion = theme.respectReducedMotion ? shouldReduceMotion : false;
  const data2 = _data.map((item) => typeof item === "string" ? {
    label: item,
    value: item
  } : item);
  ...
  const [_value, handleValueChange] = useUncontrolled({
    value,
    defaultValue,
    finalValue: Array.isArray(data2) ? (_e = (_d = (_b = data2.find((item) => !item.disabled)) == null ? void 0 : _b.value) != null ? _d : (_c = data2[0]) == null ? void 0 : _c.value) != null ? _e : null : null,
    onChange
  });
  const {
    classes,
  ...
  const controls = data2.map((item) => /* @__PURE__ */ jsxs("div", {
...
function filterData({
  ...
  const selected = value != null ? data2.find((item) => item.value === value) || null : null;
  if (selected && !filterDataOnExactSearchMatch && (selected == null ? void 0 : selected.label) === searchValue) {
    return data2;
  }
  const result = [];
  for (let i = 0; i < data2.length; i += 1) {
    if (filter(searchValue, data2[i])) {
      result.push(data2[i]);
    }
...
function defaultShouldCreate(query, data2) {
  return !!query && !data2.some((item) => item.label.toLowerCase() === query.toLowerCase());
}
const defaultProps$d = {
  required: false,
  size: "sm",
...
const Select = react.exports.forwardRef((props, ref) => {
  ...
  const formattedData = data2.map((item) => typeof item === "string" ? {
    label: item,
    value: item
  } : item);
...
const GenericService = {
  ...
  async create(resource, data2) {
    ...
    throw new Error(`cannot create resource:
         '${url}', data: '${JSON.parse(data2)}' resp: '${JSON.stringify(resp)}'`);
  },
  async update(resource, data2) {
    const url = `${config$1.baseUri}/v1/${resource}/${data2.name}`;
    ...
    throw new Error(`cannot update resource:
         '${url}', data: '${JSON.stringify(data2)}' resp: '${JSON.stringify(resp)}'`);
  },
...
const UserHooks = {
  useApiKey() {
    const { isLoading, error, data: data2, isError, refetch, isFetching, isRefetching, isSuccess, status } = useQuery(
      ["apiKey"],
    ...
    return { isLoading, isFetching, isRefetching, isSuccess, error, data: data2, isError, refetch, status };
  },
  useCurrentUser() {
    const { isLoading, error, data: data2, refetch, isSuccess } = useQuery(
      ["currentUser"],
    ...
    return { isLoading, error, data: data2, refetch, isSuccess };
  },
  ...
  useAllUsers() {
    const { isLoading, error, data: data2, refetch, isSuccess, isFetching } = useQuery(
      ["allUsers"],
    ...
    return { isLoading, error, data: data2, refetch, isSuccess, isFetching };
  }
};
...
function SafeSelect({
  ...
  const changeHandler = (event) => {
    console.log("\u{1F921}", event.target.value);
  };
  return /* @__PURE__ */ jsxs(Fragment, {
