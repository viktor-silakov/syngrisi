...
const validateAndMerge$1 = (...sources) => {
  ...
  for (const source of sources) {
    if (Array.isArray(source)) {
      ...
      if (isObject$1(source.headers)) {
        headers = mergeHeaders$1(headers, source.headers);
...
const supportsAbortController$1 = typeof globalThis.AbortController === "function";
const supportsStreams$1 = typeof globalThis.ReadableStream === "function";
const supportsFormData$1 = typeof globalThis.FormData === "function";
const requestMethods$1 = ["get", "post", "put", "patch", "head", "delete"];
...
const stop$1 = Symbol("stop");
const normalizeRequestMethod$1 = (input) => requestMethods$1.includes(input) ? input.toUpperCase() : input;
const retryMethods$1 = ["get", "put", "head", "delete", "options", "trace"];
const retryStatusCodes$1 = [408, 413, 429, 500, 502, 503, 504];
const retryAfterStatusCodes$1 = [413, 429, 503];
const defaultRetryOptions$1 = {
  limit: 2,
  methods: retryMethods$1,
  statusCodes: retryStatusCodes$1,
  afterStatusCodes: retryAfterStatusCodes$1,
  maxRetryAfter: Number.POSITIVE_INFINITY
};
const normalizeRetryOptions$1 = (retry = {}) => {
  if (typeof retry === "number") {
    return {
      ...defaultRetryOptions$1,
      limit: retry
    };
  }
...
const timeout = async (request, abortController, options) => new Promise((resolve, reject) => {
  const timeoutId = setTimeout(() => {
    ...
    reject(new TimeoutError$1(request));
  }, options.timeout);
  void options.fetch(request).then(resolve).catch(reject).then(() => {
    clearTimeout(timeoutId);
  });
});
const delay$1 = async (ms) => new Promise((resolve) => {
...
class Ky$1 {
  constructor(input, options = {}) {
    ...
    this._options = {
      ...
      ...options,
      headers: mergeHeaders$1(this._input.headers, options.headers),
      hooks: deepMerge$1({
        beforeRequest: [],
        beforeRetry: [],
        beforeError: [],
      ...
      method: normalizeRequestMethod$1((_a = options.method) != null ? _a : this._input.method),
      prefixUrl: String(options.prefixUrl || ""),
      retry: normalizeRetryOptions$1(options.retry),
    ...
    if (this._options.searchParams) {
      ...
      if ((supportsFormData$1 && this._options.body instanceof globalThis.FormData || this._options.body instanceof URLSearchParams) && !(this._options.headers && this._options.headers["content-type"])) {
  ...
  static create(input, options) {
    const ky2 = new Ky$1(input, options);
    const fn = async () => {
      ...
      if (!response.ok && ky2._options.throwHttpErrors) {
        let error = new HTTPError$1(response, ky2.request, ky2._options);
    ...
    for (const [type, mimeType] of Object.entries(responseTypes$1)) {
      result[type] = async () => {
  ...
  _calculateRetryDelay(error) {
    this._retryCount++;
    if (this._retryCount < this._options.retry.limit && !(error instanceof TimeoutError$1)) {
      if (error instanceof HTTPError$1) {
...
const createInstance$1 = (defaults) => {
  const ky2 = (input, options) => Ky$1.create(input, validateAndMerge$1(defaults, options));
  for (const method of requestMethods$1) {
    ky2[method] = (input, options) => Ky$1.create(input, validateAndMerge$1(defaults, options, { method }));
  }
  ky2.create = (newDefaults) => createInstance$1(validateAndMerge$1(newDefaults));
  ky2.extend = (newDefaults) => createInstance$1(validateAndMerge$1(defaults, newDefaults));
  ky2.stop = stop$1;
  return ky2;
};
...
function AuthFooter() {
  ...
  const {
    ...
    return ky$1(`${config.baseUri}./v1/app/info`).then((res) => {
      return res.json();
    });
  });
  if (isLoading) {
...
createInstance();
createRoot(document.getElementById("root")).render(/* @__PURE__ */ jsx(BrowserRouter, {
